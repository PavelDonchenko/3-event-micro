...
func Set(databse *Database, key string, value interface{}, expiration time.Duration) {
	...
	HandleError(err, "failed while setting value to redis database")
}
...
func Get(databse *Database, key string) string {
	...
	if err == redis.Nil {
		...
		HandleError(err, "failed while retrieving data from redis database")
	}
	return value
}

func InitRedisConn() *Database {
	database, err := NewClient(RedisUrl)
	HandleError(err, "Error occurred while connecting to redis database")
	return database
}
